import type { RouteLocationNormalizedGeneric, RouteLocationNormalizedLoadedGeneric  }vigationGuardNext } from 'vue-router'
...
export async function createPermissionGuard(to: RouteLocationNormalizedGeneric, _from: RouteLocationNormalizedLoadedGeneric, next: NavigationGuardNext) {
  ...
  if (to.name === 'Login') {
    ...
    // 有权限直接通行，无权限去到无权限页面
    hasPermission ? next() : next({ name: 'NoPermissions' })
  }
  else {
    next()
  }
