import type { AuthRoute, RouteLocationNormalizedLoaded, RouteRecordNameGeneric, RouteRecordRaw } from 'vue-router'
import useSvgIcon from '@/hooks/business/use-svg-icon'

/**
 * 筛选父级权限路由
 *
 * @param routes 所有的权限路由，可能包含子路由
 * @param roles 用户所有权限
 */
export function filterAuthRoutesByRoles(routes: AuthRoute[], roles: string[]) {
 ...
 * @param roles 用户所有权限
 */
function filterAuthRouteByRoles(route: AuthRoute, roles: string[]): AuthRoute[] {
 ...
 * @param route route
 */
function sortRouteByOrder(route: AuthRoute) {
 ...
 * @param routes routes
 */
export function sortRoutesByOrder(routes: AuthRoute[]) {
 ...
 */
export function getGlobalMenusByAuthRoutes(routes: AuthRoute[]) {
 ...
 * @param route
 */
function getGlobalMenuByBaseRoute(route: RouteLocationNormalizedLoaded | AuthRoute) {
  const { name, path } = route
  const { title, icon, localIcon, iconFontSize } = route.meta ?? {}
  ...
  const menu: App.Global.Menu = {
    ...
    routeKey: name,
    routePath: path,
    icon: useSvgIcon({ icon, localIcon, fontSize: iconFontSize || 20 }),
  }
 ...
 * @param routes
 */
export function isRouteExistByRouteName(routeName: RouteRecordNameGeneric, routes: AuthRoute[]) {
 ...
 * @param routeName
 */
function recursiveGetIsRouteExistByRouteName(route: AuthRoute, routeName: RouteRecordNameGeneric) {
