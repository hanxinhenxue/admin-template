import type { _RouteLocationBase, RouteLocationGeneric, Router } from 'vue-router'
/**
 * Get all tabs
 *
 * @param tabs Tabs
 ...
 * @param route
 */
export function getTabIdByRoute(route: _RouteLocationBase) {
 ...
 * @param route
 */
export function getTabByRoute(route: _RouteLocationBase) {
  const { name, path, fullPath = path, meta } = route

  const { title: label, fixedIndexInTab } = meta
 ...
 * @param route
 */
export function getRouteIcons(route: RouteLocationGeneric) {
...
export function getDefaultHomeTab(router: Router, homeRouteName: string) {
  const homeRoutePath = import.meta.env.VITE_ROUTE_HOME_PATH
